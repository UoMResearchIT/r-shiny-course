...
* Create and save a new Shiny app (this will be pre-filled with the example code) [git:helloworld]()
* Check this works
* Modify the app to load the ggplot2 and dplyr libraries, gapminder data and `plottingFunctions.R` [git:loadresources]()
* Modify the app to plot a histogram of all the populations in the gapminder data [git:histogram]()
...
- Create a new `sliderInput` with `inputId` "year" to select a year. [git:yearslider]()
- use the `animate` option to allow this to run through the years [git:animate]()
...
- Using the code in `codeExample.R` as a starting point,  plot the data for the selected year in the app
    - Delete the `bins` slider as it is no longer being used. [git:baseplot]()
...
- Add another widget to select continents [git:continentUI]()
- Modify your server function to filter the data so that only the selected continents are shown. [git:continent]()
...
- We make the filtered data _reactive_   [git:reactivedata]()
...
- And only then do we update the active country [git:showselected]()
...
- https://shinyapps.io easy deployment from RStudio
...
- `shinythemes::themeSelector()` widget lets us interactively exeriment with themes [git:themeselector]()
- Use the option `theme = shinytheme("themename")` on your user interface function (`fluidPage`) in our example [git:changetheme]()
...
- The widths of each row of columns should sum to 12 [git:fluidpage]()
